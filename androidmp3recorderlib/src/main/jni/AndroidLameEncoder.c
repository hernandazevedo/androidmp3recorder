#include "AndroidLameEncoder.h"
#include <jni.h>

lame_global_flags *glf;

JNIEXPORT void JNICALL Java_com_hernandazevedo_androidmp3recorder_AndroidLameEncoder_initializeDefault(
        JNIEnv *env, jclass cls) {

    glf = initializeDefault(env);
}

JNIEXPORT void JNICALL Java_com_hernandazevedo_androidmp3recorder_AndroidLameEncoder_initialize(
...
JNIEXPORT jint JNICALL Java_com_hernandazevedo_androidmp3recorder_AndroidLameEncoder_lameEncode(
...
JNIEXPORT jint JNICALL Java_com_hernandazevedo_androidmp3recorder_AndroidLameEncoder_encodeBufferInterleaved(
        JNIEnv *env, jclass cls, jshortArray pcm,
        jint samples, jbyteArray mp3buf) {
    return encodeBufferInterleaved(env, glf, pcm, samples, mp3buf);
}

JNIEXPORT jint JNICALL Java_com_hernandazevedo_androidmp3recorder_AndroidLameEncoder_lameFlush(
        JNIEnv *env, jclass cls, jbyteArray mp3buf) {
    return flush(env, glf, mp3buf);
}

JNIEXPORT void JNICALL Java_com_hernandazevedo_androidmp3recorder_AndroidLameEncoder_lameClose(
        JNIEnv *env, jclass cls) {
    close(glf);
}
